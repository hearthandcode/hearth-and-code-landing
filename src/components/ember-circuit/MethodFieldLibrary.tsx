    ...
    <dialog ref={dialogRef} className="ec-method-sheet" onClose={() => setSelected(null)} onCancel={() => setSelected(null)}>
      {selected && <article aria-labelledby={`method-sheet-${selected.id}`}>
        ...
        <div className="ec-method-sheet__example-pair">
          ...
          <section className="ec-method-sheet__output"><header><div><span>Example output</span><h3>A worked model response</h3></div><small>Generated against this card's public teaching scenario</small></header><pre>{selected.exampleOutput}</pre><footer>Illustrative output · not field evidence, validation, or a released decision.</footer></section>
        </div>
      </article>}
    </dialog>
  </>;
